                ...
                <div class="message">
                    ...
                    <div class="content-message">
						...
						<div class="content-item screen-xs24">
                            ...
                            <div class="item_time">
                                <app-count-down :count-time="paidTime" @timeOut="timeOut"></app-count-down>
                            </div>
                            </div>
                    </div>
                </div>
            ...
            <div class="verification-code">
				<div>
					...
					<div class="code-content">
						...
						<p>Make sure to send enough to cover any coin transaction fees! Only send this coin over the ETH network and </p>
						<p>make sure to use regular sends not via any kind of contract!</p>
						<div style="margin-top: 16px">
						<p>Payment ID：<span class="collapse-text-type1">{{ orderDetail.orderNo }}</span></p>
						<p>Verification Code：<span class="collapse-text-type1">{{ orderDetail.verificationCode }}</span>(save in case there are any issues with your payment)</p>
						</div>
					</div>
				</div>
				<div class="collapse">
					<van-collapse v-model="activeNames">
						<van-collapse-item title="What to do next?" name="1">
							<div class="collapse-text">1) Please send <span class="collapse-text-type1">{{ orderDetail.amount }} {{ orderDetail.symbol }}</span> to address
								<span class="collapse-text-type2">{{ orderDetail.address }}</span>.(Make sure to send enough to cover any coin transaction fees!) You will need to initiate the payment using your software or online wallet and copy/paste the address and payment amount into it. We will email you when all funds have been received.You have 3 hours,59 minutes, 54 seconds for us to receive confirmed funds.lf you send funds that don't confirm by the timeout or don't send enough coins you will receive an automatic email to claim your funds within 8 hours. lf you don't receive the email contact us with the information below and CoinPayments.net will send you a refund:
									i) The transaction ID:<span>{{ orderDetail.orderNo }}</span> ii) A payment address to send the funds to.
									iii) This verification code: <span>{{ orderDetail.verificationCode }}</span>
								...
								<div style="margin-top:16px;" class="collapse-text">
									...
									. Once the payment is confirmed several times in the block chain, the payment will be completed and the merchant will be notified. The confirmation process usually takes 10-45 minutes but varies based on the coin's target block time and number of block confirms required. The status page is available for the next 30 days.
								</div>
						</van-collapse-item>
						<van-collapse-item title="What if l accidentally don't send enough?" name="2">
							<div class="collapse-text">
								lf you don't send enough,that is OK. Just send the remainder and we will combine them for you. You can also send from multiple wallets/accounts.
							</div>
						</van-collapse-item>
					</van-collapse>
				</div>
...
export default {
    ...
    computed: {
		...
		paidTime() {
			return this.orderDetail.expireTime ? (+new Date(this.orderDetail.expireTime) - +new Date()) : 0;
		}
    },
    created() {
        this.$Progress.start();
    ...
    methods: {
        ...
        init() {
            ...
            this.getOrderDetail(param).then(res => {
                const { qrCodeUrl } = res;
                this.$nextTick(() => {
